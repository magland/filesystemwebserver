...
var common=require('../filesystemclient/common.js').common;
...
function on_request(request,callback) {
	...
	if (request.browser_code) {
		var url=wisdmconfig.filesystemwebserver.wisdmserver_url+'/wisdmserver?';
	...
	function on_request_part2() {
		...
		if (service=='filesystem') {
			filesystem(request,function(resp) {
...
function make_data_base64_url(data_base64,callback) {
	var path=wisdmconfig.filesystemwebserver.www_path+'/data_base64';
	common.mkdir(path,function() {
		...
		fs.writeFile(path+'/'+checksum+'.txt',data_base64,function(err) {
			...
			callback({success:true,url:wisdmconfig.filesystemwebserver.filesystemwebserver_url+'/data_base64/'+checksum+'.txt'});
		});
	});
}
...
var staticFileServer=new static0.Server(wisdmconfig.filesystemwebserver.www_path,{cache:3600});
	...
	if (REQ.method == 'OPTIONS') {
		...
		REQ.on('end',function(){
			...
			try {
				...
				console.error(POST);
				send_response({success:false,error:'Error parsing post: '+err.message});
				return;
			}
		...
		if (url_parts.pathname=='/filesystemwebserver') {
			...
			staticFileServer.serve(REQ,RESP,function(err,result) {
			});
...
function has_permission_to_access_file_system(NN,user_id,permissions) {
	if ((NN.owner()==user_id)&&(user_id)) return true; //owner may access always
	var access=NN.fileSystemAccess();
...
function filesystem(request,callback) {
	...
	if (command=='updateFileSystemSource') {
		if (user_id!='magland') {
			callback({success:false,error:'You are not authorized to update the file system source',authorization_error:true});
			return;
		}
		if (request.client_id=='*') {
			var pnids=FSSERVER.getConnectedFileSystemIds();
			common.for_each_async(pnids,function(pnid,cb) {
				var NN=FSSERVER.findFileSystemConnection(pnid);
				if (!NN) {
					cb({success:false,error:'Unable to find file system connection: '+pnid});
					return;
				}
			...
			var NN=FSSERVER.findFileSystemConnection(request.client_id);
			if (!NN) {
				callback({success:false,error:'Unable to find file system connection: '+request.client_id});
				return;
			}
			NN.processRequest(request,function(tmp002) {
				callback(tmp002);
	...
	if (command=='addApprovedFileSystem') {
		FSSERVER.addApprovedFileSystem(request,callback);
		return;
	}
	...
	if (!client_id) {
		callback({success:false,error:'Missing parameter: client_id'});
		return;
	}
	var NN=FSSERVER.findFileSystemConnection(client_id);
	if (!NN) {
		callback({success:false,error:'Unable to find file system: '+client_id});
		return;
	}
	
	if ((request.command||'')=='checkFileSystemConnected') {
		callback({success:true});
		return;
	}
	
	if ((request.command||'')=='getFileSystemAccess') {
		if ((NN.owner()!=user_id)||(!user_id)) {
			callback({success:false,error:'You are not authorized to get this information.'});
			return;
		}
		callback({success:true,access:NN.fileSystemAccess(),owner:NN.owner()});
		return;
	}
	...
	if (!has_permission_to_access_file_system(NN,user_id,permissions)) {
		callback({success:false,error:'You do not have permission to access this file system: '+client_id});
		return;
	}
...
//is sessionhandler needed? what does it do?
var SESSIONHANDLER=new SessionHandler();

var FSSERVER=new FileSystemServer();
FSSERVER.onSignal(function(parameters,signal) {
